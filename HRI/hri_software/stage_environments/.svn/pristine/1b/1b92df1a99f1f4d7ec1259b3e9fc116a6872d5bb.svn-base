...
StageNode::ghfunc(Stg::Model* mod, StageNode* node)
{
  ...
  else if (mod->TokenStr().find("person")==0)
...
StageNode::SubscribeModels()
{
  ...
  for (size_t r = 0; r < this->personmodels.size(); r++) {
    ROS_INFO("-- Subscribing to person %s", this->personmodels[r]->Token());
    this->personmodels[r]->Subscribe();
  }
  for (size_t r = 0; r < this->positionmodels.size(); r++)
  {
    ROS_INFO("-- Subscribing to robot %s", this->positionmodels[r]->Token());
    ...
    if (this->lasermodels.size()>r)
      laser_pubs_.push_back(n_.advertise<sensor_msgs::LaserScan>(mapName(this->laser_topic,r,static_cast<Stg::Model*>(positionmodels[r])), 10));
    ...
    say_subs_.push_back(n_.subscribe<std_msgs::String>(mapName(CMD_SAY,r,static_cast<Stg::Model*>(positionmodels[r])), 10,boost::bind(&StageNode::sayReceived, this, r, _1)));
  }
